...
export default function AnalyzeScreen() {
  ...
  const [analysisResult, setAnalysisResult] = useState<ClothingCare | null>(null);
...
const [permission, requestPermission] = useCameraPermissions();
          ...
          try {
            ...
            if (err instanceof Error) {
              if (err.name === 'NotAllowedError') {
                setError('Accès à la caméra refusé. Veuillez l\'autoriser dans les paramètres.');
              } else {
    ...
    if (cameraRef.current) {
      try {
        ...
        if (photo.base64) {
          ...
          setCapturedImages(prev => [...prev, { uri: photo.uri, base64: processedBase64 }]);
  ...
  const handleModeChange = useCallback((newMode: ScanMode) => {
    ...
    if (!isCameraAvailable && (newMode === 'garment' || newMode === 'label')) {
  ...
  const renderModeButtons = useCallback(() => (
    <View style={styles.modeButtonsContainer}>
      <Pressable
        style={[styles.modeButton, mode === 'label' && styles.activeModeButton]}
        ...
        ]}>{t('analyze.label')}</Text>
      </Pressable>

      <Pressable
        style={[styles.modeButton, mode === 'garment' && styles.activeModeButton]}
        ...
        ]}>{t('analyze.garment')}</Text>
      </Pressable>

      <Pressable
        style={[styles.modeButton, mode === 'gallery' && styles.activeModeButton]}
